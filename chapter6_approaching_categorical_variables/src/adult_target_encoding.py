...
def mean_target_encoding(data):
    ...
    features = [
        f for f in df.columns if f not in num_cols and f not in ('kfold', 'income')
    ]
    ...
    # not it is time to label encode the features
    for col in features:
        if col not in num_cols:
    ...
    for fold in range(5):
        ...
        # for all feature columns, i.e. categrocial columns
...
if __name__ == '__main__':
    # read data
    df = pd.read_csv('../input/adult_train_folds.csv')
