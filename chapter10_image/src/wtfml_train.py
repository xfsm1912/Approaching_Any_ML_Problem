...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        # here update to the new version of wtfml
        train_loss = Engine(model=model, optimizer=optimizer, device=args.device).train(
            train_loader
        )
        valid_loss = Engine(model=model, device=args.device).evaluate(
            valid_loader
        )
        print(
